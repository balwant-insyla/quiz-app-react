...
const Quiz = () => {
  ...
  function handleOptionCheckbox(e) {
    ...
    const questionIndex = resultQuestion.findIndex(
      (x) => x.question === questions[index]._id
    );
    if (questionIndex != -1) {
      const optionIndex = resultQuestion[questionIndex].options.findIndex(
        (x) => x.key === e.target.id
      );
      if (optionIndex != -1) {
        resultQuestion[questionIndex].options[optionIndex].value =
          e.target.checked;
      } else {
        resultQuestion[questionIndex].options.push({
          key: e.target.id,
      ...
      setResultQuestion([
        ...resultQuestion,
        {
          ...
          options: [{ key: e.target.id, value: e.target.checked }],
        },
      ]);
    }
    // const current = resultQuestion.
                          ...
                          >
                            <FormGroup>
                              {questions[index].options.map((option, i) => {
                                const temp = resultQuestion.find(
                                  (result) =>
                                    result.question === questions[index]._id
                                );
                                let isChecked = temp
                                  ? temp.options.find(
                                      (x) => x.key === option._id
