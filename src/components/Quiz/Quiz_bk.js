...
const Quiz = () => {
  ...
  const duration =
    subject === "Mathematics"
      ...
      : "Total Duration 5 Mins"; // I will set formula for duration.

  const quizRenderer = ({ minutes, seconds, completed }) => {
    if (completed) {
      setTimeout(true);
      saveResult();
      setResult({...result, questions: resultQuestions});
      console.log(`On checkbox change ${JSON.stringify(resultQuestions)}`);
      //setQuestions([]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!subject || !level) {
        ...
        // if(!questions) {
        //     setError('Please try again, Quiz can not prepared at the moment for you, or choose any other subject or level.');
        // }
        setError(undefined);
      }
    } catch (err) {
  ...
  function handleOptionCheckbox(e) {
    ...
    if (questionIndex != -1) {
      const optionIndex = resultQuestions[questionIndex].options.findIndex(
        (x) => x._id === e.target.id
      );
      if (optionIndex != -1) {
        resultQuestions[questionIndex].options[optionIndex].isChecked =
          e.target.checked;
       
      } else {
          
        resultQuestions[questionIndex].options.push({
          _id: e.target.id,
      ...
      setResultQuestions([
        ...resultQuestions,
        {
          ...
          options: [{ _id: e.target.id, isChecked: e.target.checked }],
        },
      ]);
    }
    
    //console.log(`On checkbox change ${JSON.stringify(resultQuestions)}`);
    //console.log(`result with questions ${JSON.stringify(result)}`);
  }

  //This method will save test result and it will called on submit button or when countdown is completed.
  const saveResult = () => {
    //console.log(`Test questions ${JSON.stringify(questions)}`);
    //console.log(`student answers to questions ${JSON.stringify(resultQuestions)}`);
    let index = 0;
    let correctAnswer = 0;
  ...
  const handleQuiz = async (e) => { //Final submission of test
    ...
    saveResult();
    //console.log(`On checkbox change ${JSON.stringify(resultQuestions)}`);
    //console.log(`result with questions, quiz submit${JSON.stringify(result)}`);
                          ...
                          >
                            <FormGroup>
                              {questions[index].options.map((option, i) => {
                                const temp = resultQuestions.find(
                                  (result) =>
                                    result._id === questions[index]._id
                                );
                                let isChecked = temp
                                  ? temp.options.find(
                                      (x) => x._id === option._id
                                  ...
                                  <FormControlLabel
                                    ...
                                    control={
                                      <Radio
                                        ...
                                        key={option._id}
                                        value={undefined}
                                        name={"option-" + questions[index]._id}
